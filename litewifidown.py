...
choose= choose + 1
choose=str(choose)
os.system('echo "'+choose+'p; d'" > choose')
os.system('card=`cat card.sh` && choose=`cat choose` && nmcli -f NAME,BSSID,CHAN,RATE,SIGNAL,SSID, dev wifi list ifname $card | awk '{print FS3 $3}' | sed '$choose ')
