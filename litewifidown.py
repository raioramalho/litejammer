...
if way == 1:
  ...
  os.system("card=`cat card.sh` && choose=`cat choose` && nmcli -f NAME,BSSID,CHAN,RATE,SIGNAL,SECURITY,SSID, dev wifi list ifname $card | awk '{print FS2 $2}' | sed '/$choose/p; d' > targ")
  #os.system('card=`cat card.sh` && ch=`cat chan` && airmon-ng start $card $ch')
  os.system('card=`cat card.sh`mon && choose=`cat choose` && target=`cat targ` && echo $target')
  print("Wait..")
  #os.system('card=`cat card.sh`mon && ch=`cat chan` && airmon-ng stop $card')
