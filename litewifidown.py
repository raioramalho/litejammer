...
os.system('echo "p" > p')
os.system('card=`cat card.sh` && p=`cat p` && choose=`cat choose` && nmcli -f NAME,BSSID,CHAN,RATE,SIGNAL,SSID, dev wifi list ifname wlan1 | awk '{print FS3 $3}' | sed '$choose$p; d'')
